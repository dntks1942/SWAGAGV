...
public class ControlPanel
   ...
   */
  private RealAGVDemoProcessModelTO processModel;
   ...
   */
  @Inject
  public ControlPanel(@Assisted RealAGVDemoProcessModelTO processModel,
  ...
  private void initGuiContent() {
    ...
    for (RealAGVDemoProcessModel.Attribute attribute : RealAGVDemoProcessModel.Attribute.values()) {
      processModelChange(attribute.name(), processModel);
    }
  }
  ...
  public void processModelChange(String attributeChanged, VehicleProcessModelTO newProcessModel) {
    if (!(newProcessModel instanceof RealAGVDemoProcessModelTO)) {
      return;
    }
    processModel = (RealAGVDemoProcessModelTO) newProcessModel;
    ...
    else if (Objects.equals(attributeChanged, RealAGVDemoProcessModel.Attribute.VEHICLE_HOST.name())) {
      updateVehicleHost(processModel.getVehicleHost());
    }
    else if (Objects.equals(attributeChanged, RealAGVDemoProcessModel.Attribute.VEHICLE_PORT.name())) {
      updateVehiclePort(processModel.getVehiclePort());
    }
    else if (Objects.equals(attributeChanged,
                            RealAGVDemoProcessModel.Attribute.VEHICLE_IDLE_TIMEOUT.name())) {
      updateVehicleTimeout(processModel.getVehicleIdleTimeout());
    }
    else if (Objects.equals(attributeChanged, RealAGVDemoProcessModel.Attribute.VEHICLE_IDLE.name())) {
      updateVehicleIdle(processModel.isVehicleIdle());
    }
    else if (Objects.equals(attributeChanged,
                            RealAGVDemoProcessModel.Attribute.DISCONNECTING_ON_IDLE.name())) {
      updateDisconnectingOnIdle(processModel.isDisconnectingOnVehicleIdle());
    }
    else if (Objects.equals(attributeChanged,
                            RealAGVDemoProcessModel.Attribute.RECONNECTING_ON_CONNECTION_LOSS.name())) {
      updateReconnectingOnConnectionLoss(processModel.isReconnectingOnConnectionLoss());
    }
    else if (Objects.equals(attributeChanged, RealAGVDemoProcessModel.Attribute.LAST_ORDER.name())) {
      updateLastOrder(processModel.getLastOrderSent(), processModel.isCommAdapterConnected());
    }
    else if (Objects.equals(attributeChanged,
                            RealAGVDemoProcessModel.Attribute.LOGGING_ENABLED.name())) {
